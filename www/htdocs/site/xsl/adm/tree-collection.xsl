...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="doc-file">
			<xsl:choose>
				<xsl:when test="$id != ''">
					<xsl:value-of select="document($doc-name)/bvs/*[name() = $find-element]//item[@id = $id]/@file"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="document($doc-name)/bvs/*[name() = $find-element]//item[@type = $page]/@file"/>
				</xsl:otherwise>
			</xsl:choose>
	</xsl:variable>	
