...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		...
		<div id="decsBase">
			...
			<span>
				<a href="#" onclick="window.close()"><xsl:value-of select="$texts/text[@id = 'close']"/>&#160;<img src="../image/common/close.gif"/></a>
			</span>			
		</div>
	...
	<xsl:template match="allowable_qualifier_list">
		...
		<tr class="evenLine">
			<td class="label"><xsl:value-of select="$texts/text[@id = 'allowable_qualifier_list']" /></td>
			<td valign="top">			
			<table>
			<xsl:for-each select="$translated_qualifier_list/allowable_qualifier_list/allowable_qualifier">
				<xsl:sort select="."/>			
				<tr>
					<td><a href="#" onclick="showDeCSQualifier('{@qualifier_id}')"><xsl:value-of select="."/></a></td>
				</tr>	
			</xsl:for-each>
			</table>			
			</td>
