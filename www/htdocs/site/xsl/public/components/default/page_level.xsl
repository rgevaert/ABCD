...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="item" mode="breadCrumb">
		<a href="../php/level.php?lang={$lang}&amp;component={$component}&amp;item={@id}"><xsl:apply-templates select="text()" /></a>
		<xsl:if test="position() != last()"> &gt; </xsl:if>
	</xsl:template>
	
	<xsl:template match="item[@img]" mode="breadCrumb">
		<a href="../php/level.php?lang={$lang}&amp;component={$component}&amp;item={@id}"><img src="{@img}" alt="text()" /></a> 
		<xsl:if test="position() != last()"> &gt; </xsl:if>
	</xsl:template>
	
	<xsl:template match="collection//item | community//item | about//item | portal//item">
		<li>
			<h5><a href="../php/level.php?lang={$lang}&amp;component={$component}&amp;item={@id}"><xsl:apply-templates select="text()" /></a></h5>
