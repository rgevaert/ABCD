...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
			...
			<h3>
			 <span>
			  <xsl:choose>
				...
				<xsl:otherwise>
					<a href="../php/level.php?lang={$lang}&amp;component={$id}"><xsl:value-of select="$portalListData/text()" /></a>
				</xsl:otherwise>
			  </xsl:choose>	
			 </span>
			</h3>			
	...
	<xsl:template match="portal//item[item and @img]">
		<li>
			<a href="../php/level.php?lang={$lang}&amp;component={$id}&amp;item={@id}"><img src="{@img}" alt="{text()}" /></a>
			<xsl:if test="ancestor::item"> + </xsl:if>
			<ul>
				<xsl:comment>item</xsl:comment>
				<xsl:apply-templates select="item" />
