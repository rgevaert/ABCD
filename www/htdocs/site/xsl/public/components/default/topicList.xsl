			...
			<h3>
			 <span>
			  <xsl:choose>
				...
				<xsl:otherwise>
					<a href="../php/level.php?lang={$lang}&amp;component={$id}"><xsl:value-of select="$topicData/text()" /></a>
				</xsl:otherwise>
			  </xsl:choose>	
			 </span>
			</h3>						
		...
		<li>
			<xsl:choose>
				<xsl:when test="meta-search/info-source">
					<a href="{$metaIAH}?lang={$lang}&amp;topic={concat($id,'-',@id)}"><xsl:apply-templates select="text()"/></a>					
				</xsl:when>
				<xsl:otherwise>
