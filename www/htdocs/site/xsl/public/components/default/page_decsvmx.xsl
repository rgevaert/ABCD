...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="*" mode="basket">
		...
		<div id="searchButtonEnabled">
			...
			<input type="radio" name="connector" value="AND" checked="checked"><xsl:value-of select="$texts/text[@id = 'and_terms']"/></input>
			<input type="radio" name="connector" value="OR"><xsl:value-of select="$texts/text[@id = 'or_terms']"/></input>
		...
		<div id="searchButtonDisabled">
			...
			<input type="radio" name="connector" value="AND" disabled="true"><xsl:value-of select="$texts/text[@id = 'and_terms']"/></input>
			<input type="radio" name="connector" value="OR" disabled="true"><xsl:value-of select="$texts/text[@id = 'or_terms']"/></input>
	...
	<xsl:template match="session/terms">
		...
		<xsl:variable name="sessionTerm">
			<xsl:call-template name="split">
				<xsl:with-param name="string" select="."/>
			</xsl:call-template>		
		</xsl:variable>

		<xsl:variable name="id" select="$sessionTerm/token[1]"/>
		<xsl:variable name="name" select="$sessionTerm/token[2]"/>
		<xsl:variable name="options" select="$sessionTerm/token[3]"/>
		<xsl:variable name="qualifier" select="$sessionTerm/token[4]"/>
