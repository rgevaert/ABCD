...
//    msgwin=window.open("campos.php?wks="+Wks+"&tag="+TagCampo+"&base="+base,"Window1" ,"status=yes,resizable=yes,toolbar=false,menu=no,scrollbars=yes,,height=500,top=50,left=0")
    msgwin=window.open("campos.php?tag="+TagCampo+"&base="+base+wks,"Window1" ,"status=yes,resizable=yes,toolbar=false,menu=no,scrollbars=yes,,height=500,top=50,left=0")
    msgwin.focus()


}

function CampoFijo(Tag,i,Formato,Base,Tml){
	ix=document.forma1.tag3006.selectedIndex
	if (ix<1) {
		alert("Please select a type of record")
		return
	}
	Formato=document.forma1.tag3006.options[ix].value
	NombreC=Tag.name
    Eti=Tag.name
	TagCampo=Eti.substr(3)
	Eti="eti"+TagCampo
    ...
    document.forma1.SubC.value=a
    msgwin=window.open("campofijo.php?formato="+Formato+"&base="+Base+"&Tag="+NombreC+"&tm="+Tml,"Window1" ,"status=yes,resizable=yes,toolbar=false,menu=no,scrollbars=yes,width=750,height=500,top=50,left=0")
    msgwin.focus()
