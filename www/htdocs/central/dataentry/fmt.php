...
//	$Expresion=str_replace("\'","'",$Expresion);
	...
	$query = "&base=".$arrHttp["base"] ."&cipar=$db_path"."par/".$arrHttp["cipar"]."&Expresion=".$Expresion."&count=1&from=".$arrHttp["from"]."&Formato=$Formato";
	include("../common/wxis_llamar.php");
    $ficha_bib=$contenido;
	foreach ($ficha_bib as $linea){		$linea=trim($linea);
		if ($linea!="") {
			if (substr($linea,0,6)=='$$REF:'){
 				...
 				$query = "&cipar=$db_path"."par/".$arrHttp["cipar"]. "&Expresion=".$expr_ref."&Opcion=buscar&base=" .$bd_ref."&Formato=$pft_ref.pft";
				include("../common/wxis_llamar.php");
...
if ($arrHttp["Opcion"]=="ver" or $arrHttp["Opcion"]=="cancelar" or $arrHttp["Opcion"]=="buscar" or ($arrHttp["Opcion"]=="actualizar") ) {
...
if (isset ($arrHttp["Opcion"]))  {
	if ($arrHttp["Opcion"]=="actualizar" || $arrHttp["Opcion"]=="actualizarregistrousuario" ) $actualizar="SI";
