...
foreach ($arrHttp as $var=>$value){	if (substr($var,0,3)=="tag"){		$tag=trim(substr($var,3));
		for ($i=strlen($tag);$i<4;$i++) $tag="0".$tag;
		if ($ValorCapturado=="")
			$ValorCapturado=$tag.trim($value);
		else
			$ValorCapturado.="\n".$tag.trim($value);
	}}
//ADD THE CONTROL NUMBER TO THE UPDATE STRING
for ($i=strlen($tag_ctl);$i<4;$i++) $tag_ctl="0".$tag_ctl;
$ValorCapturado.="\n".$tag_ctl.$CN;
...
function ReadPurchaseOrder(){	global $arrHttp,$msgstr,$db_path,$Wxis,$xWxis;
...
// GET THE FST TO FIND THE CONTROL NUMBER OF A BIBLIOGRAPHIC DATABASE
	...
	if (!file_exists($archivo)){
		echo $msgstr["missing"].": ".$base."/data/".$base.".fst";
		die;
	}
	$fp=file($archivo);
	$tag_ctl="";
