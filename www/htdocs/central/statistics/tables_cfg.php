 ...
 * @file:      tables_cfg.php
 * @desc:      CONFIGURE THE LIST OF TABLES TO GENERATE GRAPHICS
 *             Read the list of defined variables from the "stat.cfg" file in the
 *             def directory of the database. If the "stat.cfg" is missing, execution stops.
...
function DrawElement(ixEl,Title,ixRow,ixCol){
    ...
    nuevo+="</select></td>"
    nuevo+="<tr><td bgcolor=white><?php echo $msgstr["column"]?></td><td bgcolor=white><select name=cols><option></option>"
    ix0=0
    for (var opt in f){
    	ix0++
    	selected=""
...
foreach ($fp as $value) {
	...
	if ($value!=""){
		...
		echo "<td bgcolor=white><input type=text name=tit size=120 value=\"".$t[0]."\"></td>";
   		...
   		echo "</select></td>";
   		echo "<tr><td bgcolor=white>".$msgstr["column"]."</td><td bgcolor=white><select name=cols><option></option>";
   		$f=explode('||',$fields);
   		foreach ($f as $opt) {
   			$selected="";
