...
foreach ($fp as $value) {
	if (!empty($value)) {
		...
		echo "
		item[$ix]=\"".trim($value)."\"
	//	item[$ix][1]=\"".$type_items[$Ti[0]]." - ".$Ti[1]."\"\n";
    }
}
echo "</script>\n";
?>
...
?>
    ...
    function Aceptar_Item(Index){
    	...
    	if (i<0){
    		alert("<?php echo $msgstr["falta"].": ".$rows_title[0]?>")
    		return
    	}
    	...
    	if (i<0){    		alert("<?php echo $msgstr["falta"]." ".$rows_title[1]?>")
    		return    	}
...
//    		return
    	...
    	if (document.forma1.reserva.checked)
    		x=Trim(document.forma1.reserva.value)    	else    		x=""    	xItem+='|'+x
    	...
    	if (x!=""){    		res=ValidarFecha(x.substr(4,2)+"/"+x.substr(6,2)+"/"+x.substr(0,4))
    		if (res==false){
    			alert("<?php echo $msgstr["invaliddate"].": ".$rows_title[15]?>")
    			return
    		}
    	}
    	xItem+='|'+x
    	...
    	if (x!=""){
	    	res=ValidarFecha(x.substr(4,2)+"/"+x.substr(6,2)+"/"+x.substr(0,4))
	    	if (res==false){	    		alert("<?php echo $msgstr["invaliddate"].": ".$rows_title[16]?>")
	    		return
	   		}    	}
    	xItem+='|'+x
	...
	function Redraw(sel,index){    	elem = document.getElementById("type_e");
    	...
    	for (var c in RT){
    		...
    		switch (c){    			case "0":
    			...
    			case "11":
    				chk=""
					if (cell[c]=="Y") chk=" checked"    				html+="<input type=checkbox name=reserva value=\"Y\" "+chk+"></td>"    				break
				case "12":
					chk=""
				...
				case "14":
				    ...
				    break
				case "15":
    				html+="<input type=text name=fecha_u value=\""+cell[c]+"\" size=8> "+"<?php echo $msgstr['it_iso']?>"+" (YYYYMMDD)</td>"
    				break
    			case "16":
    				html+="<input type=text name=fecha_i value=\""+cell[c]+"\" size=8> "+"<?php echo $msgstr['it_iso']?>"+" (YYYYMMDD)</td>"
    				break
				case "17":
					chk=""
		    	...
		    	for (var c in cell){
		    		...
		    		switch (c){
		    			...
		    			case "16":
		    				html+="<td>"+cell[c]+"</td>"
		    				break
		  				case "11":		    				chk=""							if (cell[11]=="Y") chk="Y"						    html+="<td>"+chk+"</td>"						    break						case "5":
							unidad=""
						...
						case "12":
							chk=""
							if (cell[12]=="Y") chk="<?php echo $msgstr['yes']?>"
						    html+="<td>"+chk+"</td>"
						    break;
						case "13":
							chk=""
							if (cell[13]=="Y") chk="<?php echo $msgstr['yes']?>"
						    html+="<td>"+chk+"</td>"
						    break
						case "14":
							chk=""
							if (cell[14]=="Y") chk="<?php echo $msgstr['yes']?>"
						    html+="<td>"+chk+"</td>"
						    break
						case "17":
							chk=""
							if (cell[17]=="Y") chk="<?php echo $msgstr['yes']?>"
						    html+="<td>"+chk+"</td>"
						    break
		    		}
                }
...
function isDate(dtStr){
	...
	year=parseInt(strYr)
	if (pos1==-1 || pos2==-1){
		alert('<?php echo $msgstr["dateformat"]." : mm/dd/yyyy"?>')
		return false
	}
	if (strMonth.length<1 || month<1 || month>12){
		alert('<?php echo $msgstr["enterval"]." ".$msgstr["mes"]?>')
		return false
	}
	if (strDay.length<1 || day<1 || day>31 || (month==2 && day>daysInFebruary(year)) || day > daysInMonth[month]){
		alert('<?php echo $msgstr["enterval"]." ".$msgstr["dia"]?>')
		return false
	}
	if (strYear.length != 4 || year==0 || year<minYear || year>maxYear){
		alert('<?php echo $msgstr["enterval"]." ".$msgstr["ano"]." ".$msgstr["from"]?> '+minYear+' <?php echo $msgstr["to"]?> '+maxYear)
		return false
	}
	if (dtStr.indexOf(dtCh,pos2+1)!=-1 || isInteger(stripCharsInBag(dtStr, dtCh))==false){
		alert('<?php echo $msgstr["invaliddate"]?>')
		return false
	}
return true
}
