...
global $db_path,$Wxis,$xWxis,$arrHttp,$cn,$db,$item,$multa;
...
global $db_path,$Wxis,$xWxis,$arrHttp,$pft_totalitems,$pft_in,$pft_nc,$pft_typeofr,$titulo,$prefix_in,$multa;
...
function Reservas($signatura){global $db_path,$Wxis,$xWxis,$msgstr;
// se presenta la lista de reserva
	...
	foreach ($contenido as $linea){
 	...
 	if (count($reserva)>0){ 		echo "<h5>".$msgstr["reservas"]."</h5>";
		...
		echo "<tr><th>".$msgstr["delete"]."</th><th>".$msgstr["users"]."</th><th>".$msgstr["signature"]."</th><th>".$msgstr["r_date"]."</th><th>".$msgstr["typeofitems"]."</th>";
