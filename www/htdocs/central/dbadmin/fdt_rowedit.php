	...
	function Picklist(name,row){
		...
		switch (pl_type){
			...
			case "D":
				...
				if (dbsel=="") dbsel="<?echo $arrHttp["base"]?>"
				prefix=document.forma1.c13.value
				list=document.forma1.c14.value
				extract=document.forma1.c15.value
				Url="picklist_db.php?base=<?echo $arrHttp["base"]?>&picklist="+name+"&row="+row +"&dbsel="+dbsel+"&prefix="+prefix+"&list="+list+"&extract="+extract
				break
...
if (trim($Fdt[0])!="H" and trim($Fdt[0])!="L"){
	if ($Fdt[10]=="")
		$pick="<a href='javascript:Picklist(\"".$Fdt[10].".tab\",".$arrHttp["row"].")'><font size=1>".$msgstr["browse"]."</a>";
	else
		$pick="<a href='javascript:Picklist(\"".$Fdt[10]."\",".$arrHttp["row"].")'><font size=1>".$msgstr["browse"]."</a>";
}
echo "$pick</td>";
