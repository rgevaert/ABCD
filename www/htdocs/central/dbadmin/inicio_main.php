 ...
 * @file:      inicio_main.php
 * @desc:      Start the program with user-level
...
// identificada entre $$LLAVE= .....$$
 ...
 $llave_pft="";
 global $llamada, $valortag,$maxmfn,$arrHttp,$OS,$Bases,$xWxis,$Wxis,$Mfn,$db_path;
...
if (isset($arrHttp["base"])){
	...
	if (empty($Permiso)){
		echo "<?php echo $msgstr["menu_noau"]?>";
		session_destroy();
		die;
	}
...
function ApagarEdicion(){
	...
	item.disable();
	var item=top.menu.toolbar.getItem("4_cancelar"); //get item object by
	item.disable();
	var item=top.menu.toolbar.getItem("4_guardar"); //get item object by
	item.disable();
	var item=top.menu.toolbar.getItem("4_eliminar"); //get item object by
	item.disable();
...
function TipoDeRegistro(){
	...
	top.main.document.writeln("<h4><?php echo $msgstr['seleccionar'].' '.$msgstr['typeofr']?></h4><table>")
...
function Permisos(){
	if (db_permiso!="adm" && db_permiso!="dbadm" && db_permiso!="dboper" ){
		alert("<?php echo $msgstr["noeditdb"]?>")
		return  false
	}else{
		return true
	}
...
function ValidarIrA(){
	...
	if (blnResult==false){
		alert("<?echo $msgstr["especificarvaln"]?>")
		return false
	}
	if (xmfn>maxmfn){
	  	alert("<?echo $msgstr["numfr"]?>")
	  	return false
	}
	return xmfn
}
...
function Menu(Opcion){
	...
	switch (Opcion) {
		...
		case 'alfa':
			...
			var width = screen.width-500-100
...
if (isset($arrHttp["base"])){
    ...
    echo "<frame name=encabezado src=menubases.php?Opcion=Menu_o&base=acces&cipar=cipar.par&Mfn=".$arrHttp["Mfn"]."&base_activa=$bd&per=$bdright MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING=no FRAMEBORDER=NO>\n";
	echo "<frame name=menu  src=\"menu_main.php?\" scrolling=no frameborder=NO  marginheight=0   MARGINWIDTH=0 >";
	echo "<frame name=main  src=\"inicio_base.php?base=$bd"."&cipar=$bd".".par&per=$bdright\" scrolling=no frameborder=NO  marginheight=0   MARGINWIDTH=0 >";
}else{
	echo "<frame name=encabezado src=menubases.php?Opcion=Menu_o&base=acces&cipar=cipar.par&Mfn=".$arrHttp["Mfn"]." MARGINWIDTH=0 MARGINHEIGHT=0 SCROLLING=no FRAMEBORDER=NO>\n";
